...
### Changed
- Published the retirement maintenance notice to npm and the MCP Registry.
  Public hosted access retired July 28, 2026; this package remains available
  for Ryan McMillan's private deployment and as a verifiable engineering
  artifact. Registry metadata now points to the canonical portfolio case study
  and describes existing owner credentials as the only hosted access path.
